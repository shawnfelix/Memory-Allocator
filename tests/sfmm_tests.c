...
//Test that you can add blocks bigger than size of page
Test(sf_memsuite_student, malloc_invalid_pointer_test, .init = sf_mem_init, .fini = sf_mem_fini) {
	int *x = sf_malloc(sizeof(int)*2000);
	sf_free(x);

	assert_free_block_count(8144,1);
}

//Test if the payload is successfully set when after subsequent calls to malloc and free
Test(sf_memsuite_student, malloc_payload_test, .init = sf_mem_init, .fini = sf_mem_fini) {
	int *x = sf_malloc(sizeof(int) * 20);

	*x = 10;
	void *y = sf_malloc(sizeof(int) * 500);
	y = sf_realloc(y, sizeof(int));

	int *t = sf_malloc(sizeof(int) * 10);
	sf_free(t);
	sf_free(y);

	cr_assert_eq(10, *x, "The payload is corrupt after calls to malloc and free! Expected: 10, Got:%d", *x);
}
//Test if the block is freed if a pointer is passed but size is zero.
Test(sf_memsuite_student, realloc_valid_pointer_size_zero, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(sizeof(int) * 10);

	x = sf_realloc(x, 0);
	assert_free_block_count(0,1);
}

//Test if the payloads are the same after reallocing
Test(sf_memsuite_student, realloc_payload_test, .init = sf_mem_init, .fini = sf_mem_fini) {
	int *ptr = sf_malloc(128);
	int *ptr2 = sf_malloc(50);

	*ptr = 10;
	*ptr2 = 4;

	ptr = sf_realloc(ptr, 200);
	cr_assert_eq(10, *ptr, "The payload is corrupted after sf_realloc()!%d", *ptr);
}

//Test that the payload isnt corupt for a block that is smaller
Test(sf_memsuite_student, realloc_free_valid_pointer_test , .init = sf_mem_init, .fini = sf_mem_fini) {
	int *x = sf_malloc(200);
	int *y = sf_malloc(200);
	...
	*y = 5;
	...
	z = z;
	cr_assert_eq(5, *y, "The payload is corrupted after sf_realloc()!%d", *y);
}
